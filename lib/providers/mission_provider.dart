...
class MissionNotifier extends StateNotifier<List<MissionData>> {
  ...
  void _initializeMissions() {
    state = [
      ...
      MissionData(id: '2', title: 'ストリーク7日', description: '7日連続ログインしよう', reward: 100, progress: 3, target: 7, isCompleted: false),
      MissionData(id: '3', title: 'レベルアップ', description: 'レベルを1上げよう', reward: 75, progress: 0, target: 1, isCompleted: false),
    ];
  }
  ...
  void updateProgress(String missionId, int progress) {
    state = [
      for (final m in state)
        if (m.id == missionId) m.copyWith(progress: progress, isCompleted: progress >= m.target) else m,
    ];
  }
}
